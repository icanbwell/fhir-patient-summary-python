...
Pipfile.lock:
	...
	docker compose --progress=plain run --rm dev sh -c "cp -f /tmp/Pipfile.lock /usr/src/fhirpatientsummary/Pipfile.lock"
...
.PHONY:tests
tests: up
	docker compose run --rm --name fhirpatientsummary dev pytest tests fhirpatientsummary

.PHONY:shell
shell:devdocker ## Brings up the bash shell in dev docker
	docker compose run --rm --name fhirpatientsummary dev sh

.PHONY:build
build:
	docker compose run --rm --name fhirpatientsummary dev rm -rf dist/
	docker compose run --rm --name fhirpatientsummary dev python3 setup.py sdist bdist_wheel

.PHONY:testpackage
testpackage:build
	docker compose run --rm --name fhirpatientsummary dev python3 -m twine upload -u __token__ --repository testpypi dist/*
# password can be set in TWINE_PASSWORD. https://twine.readthedocs.io/en/latest/

.PHONY:package
package:build
	docker compose run --rm --name fhirpatientsummary dev python3 -m twine upload -u __token__ --repository pypi dist/*
