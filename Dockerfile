...
WORKDIR /usr/src/fhirpatientsummary/
...
COPY Pipfile* /usr/src/fhirpatientsummary/
...
ENV PROJECT_DIR=/usr/src/fhirpatientsummary
ENV FLASK_APP=fhirpatientsummary.api
...
COPY ./fhirpatientsummary ${PROJECT_DIR}

# Copy the Pipfile.lock from the previous stage in case it was locked
COPY --from=python_packages /usr/src/fhirpatientsummary/Pipfile.lock /usr/src/fhirpatientsummary/Pipfile.lock

# Copy Pipfile.lock to a temporary directory so it can be retrieved if needed
COPY --from=python_packages /usr/src/fhirpatientsummary/Pipfile.lock /tmp/Pipfile.lock
